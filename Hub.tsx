    ...
    >
          ...
          >
            {BANNERS.map((banner, i) => (
              <div 
                ...
                style={{ width: '100%' }}
              >
                <div className={`${banner.bg} p-6 md:p-8 rounded-[16px] border border-hairline shadow-sm shadow-black/20 h-48 md:h-56 flex flex-col justify-between`}>
                  <div>
                    <h3 className="text-2xl font-bold text-white mb-2">{banner.title}</h3>
                    <p className="text-[14px] text-muted max-w-sm line-clamp-2">
                      {banner.description}
                    </p>
                  </div>
      ...
      <div className="px-4 md:px-8">
         ...
         <div className="grid grid-cols-2 gap-4">
            <button 
              onClick={() => onNavigate('catalog')}
              className="bg-surface p-4 rounded-[16px] border border-hairline text-left flex flex-col gap-2 hover:bg-white/5 transition-colors"
            >
              <div className="w-10 h-10 rounded-full bg-brand/10 flex items-center justify-center">
                 <div className="w-4 h-4 bg-brand rounded-sm" />
              </div>
              <span className="font-semibold text-white">Browse Catalog</span>
              <span className="text-xs text-muted">Explore all digital items</span>
            </button>
            <button 
            ...
            >
              ...
              <span className="text-xs text-muted">View your collection</span>
            </button>
         </div>
      </div>
    </motion.div>
