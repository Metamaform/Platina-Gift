...
import { generateCleanPreview } from '../lib/lExtractor'; // Wait, it's lottieExtractor
import { Loader2, AlertCircle, Filter, X, Search, Check, PackageOpen, ArrowDownUp } from 'lucide-react';
...
function TraitPreview({ trait, value, sampleUrl, color }: { trait: string, value: string, sampleUrl?: string, color?: string }) {
  ...
  if (trait === 'Backdrop') {
    return <div className="w-10 h-10 rounded-[12px] flex-shrink-0 border border-white/5" style={{ backgroundColor: color || '#333' }} />;
  }
  
  if (img) {
    return (
      <div className="w-10 h-10 flex items-center justify-center flex-shrink-0 bg-white/5 rounded-[12px] border border-white/5 p-1">
         <img src={img} className="w-full h-full object-contain" alt={value} />
      </div>
    );
  }
  
  return <div className="w-10 h-10 rounded-[12px] bg-white/5 animate-pulse flex-shrink-0 border border-white/5" />;
}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col justify-end">
      ...
      <motion.div 
        ...
        className="relative w-full bg-[#15151C] rounded-t-3xl border-t border-white/5 overflow-hidden flex flex-col max-h-[85vh]"
      >
        ...
        <div className="px-4 pb-4">
          <h3 className="text-xl font-bold mb-4">{trait}s</h3>
          
          <div className="relative">
            <Search className="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-[#9A9AA5]" />
            <input 
              type="text" 
              placeholder={\`Search \${trait.toLowerCase()}...\`}
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="w-full bg-[#0D0D12] rounded-2xl pl-10 pr-4 py-3 text-[15px] text-white placeholder:text-[#9A9AA5] focus:outline-none focus:ring-1 focus:ring-purple-500"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-safe custom-scrollbar">
          ...
          >
            <div className="w-10 h-10 rounded-[12px] bg-[#0D0D12] border border-white/5 flex items-center justify-center flex-shrink-0">
               <div className="w-3 h-3 rounded-full bg-white/30" />
            </div>
            <span className="text-[16px] font-semibold flex-1 text-left text-white">All {trait}s</span>
            <span className="text-[13px] text-[#9A9AA5]">{totalItems}</span>
            {!selectedValue && <Check className="w-5 h-5 text-purple-400 ml-2" />}
          </button>
          ...
          {filteredOptions.map(opt => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[13px] text-[#9A9AA5]">{opt.count}</span>
                {isSelected && <Check className="w-5 h-5 text-purple-400 ml-2" />}
              </button>
            );
          })}
        </div>
...
// GIFT CARD COMPONENT
// ----------------------------------------------------------------------
function GiftCard({ item }: { item: NFTItem }) {
  ...
  return (
    <motion.div
      ...
      className="rounded-[16px] overflow-hidden bg-[#15151C] border border-white/5 shadow-sm shadow-black/20 flex flex-col"
    >
      <div className="aspect-square w-full relative bg-[#0D0D12] p-2 flex items-center justify-center">
        {isHovered && lottieUrl ? (
      ...
      <div className="p-3">
        ...
        <p className="text-[13px] text-[#9A9AA5] mt-1">Collectible</p>
      </div>
    </motion.div>
  );
}
    ...
    <div className="w-full max-w-7xl mx-auto p-4 md:p-6 space-y-6">
      ...
      {mode === 'catalog' ? (
        <div className="space-y-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-[#15151C] rounded-2xl pl-10 pr-4 py-3 text-[15px] text-white placeholder:text-[#9A9AA5] focus:outline-none border border-white/5 focus:border-purple-500/50"
            />
          </div>
            ...
            {['newest', 'name', 'type'].map(sortOpt => (
              <button 
                ...
                className={\`px-4 py-2 rounded-full text-[13px] font-semibold whitespace-nowrap border transition-colors \${
                  sortBy === sortOpt ? 'bg-purple-500/20 text-purple-400 border-purple-500/30' : 'bg-[#15151C] text-[#9A9AA5] border-white/5'
                }\`}
              >
        ...
        <div className="space-y-4">
              ...
              <button
                ...
                className={\`shrink-0 px-4 py-2 rounded-full text-[14px] font-semibold transition-colors border \${
                  selectedCollection === collection.address
                    ...
                    : 'bg-[#15151C] text-[#9A9AA5] hover:text-white border-white/5'
                }\`}
              >
                {collection.name}
              </button>
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
            <div className="flex items-center justify-center w-9 h-9 rounded-full bg-[#15151C] border border-white/5 flex-shrink-0 text-[#9A9AA5]">
               ...
               return (
                 <button
                   ...
                   className={\`px-4 py-2 rounded-full text-[13px] font-semibold whitespace-nowrap border transition-colors flex items-center gap-1 \${
                     selectedVal ? 'bg-purple-500/20 text-purple-400 border-purple-500/30' : 'bg-[#15151C] text-[#9A9AA5] border-white/5'
                   }\`}
                 >
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          <motion.div key="loader" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex justify-center py-20">
            <Loader2 className="w-8 h-8 text-[#9A9AA5] animate-spin" />
          </motion.div>
        ) : error ? (
          ...
          <motion.div key="grid" variants={{ show: { transition: { staggerChildren: 0.05 } } }} initial="hidden" animate="show">
            {filteredAndSortedItems.length === 0 ? (
              <div className="py-20 flex flex-col items-center text-center">
                <PackageOpen className="w-16 h-16 text-[#9A9AA5]/30 mb-4" />
                <h3 className="text-lg font-bold text-white mb-2">No items found</h3>
                <p className="text-[14px] text-[#9A9AA5]">Try adjusting your search or filters.</p>
              </div>
            ) : (
            ...
            {visibleCount < filteredAndSortedItems.length && (
              <div className="flex justify-center pt-8">
                <button
                  ...
                  className="px-6 py-3 rounded-full bg-[#15151C] border border-white/5 text-[14px] font-bold text-white transition-colors hover:bg-white/5"
                >
                  Load More
                </button>
              </div>
      ...
      <AnimatePresence>
        {activeSheet && attributesMap[activeSheet] && (
          <BottomSheetFilter 
            key={activeSheet}
            trait={activeSheet}
            options={Array.from(attributesMap[activeSheet].values()).sort((a,b) => b.count - a.count)}
